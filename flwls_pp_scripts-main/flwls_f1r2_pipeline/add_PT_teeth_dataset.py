...
def import_alembic_animation_cache(filepath, framerate=24):
    alembic_node = nuke.createNode('ReadGeo2', f'file {filepath}', inpanel=False)
    alembic_node.forceValidate()
    alembic_node.knob('frame_rate').setValue(framerate)

    # Make sure the node displays the entire scene graph
    scene_view = alembic_node.knob('scene_view')
...
def gen_dataset(gizmo_path = '/Volumes/f1r2/pipeline/vfx/templates/fotd_render_dataset/fotd_render_dataset_template_v007.gizmo'):
    ...
    if character == '101':
        ...
        checkpoint = '/Volumes/f1r2/sean/investigations/teeth/john/mouth_geom_checkpoints/model.014-0.00381964.ckpt'
    else:
    ...
    base_geometry_path = '/Volumes/f1r2/Projects/squd01/episodes/squd01_ep99/parts'
    teeth_geo_dir = os.path.join(base_geometry_path, part, 'shots', shot, 'language', language, 'performanceTransfer', character, variant, 'publish', 'TEETH')

    teeth_geo_latest_version = 0
    for vstring in os.listdir(teeth_geo_dir):
        v = int(vstring[1:])
        if v > teeth_geo_latest_version:
            teeth_geo_latest_version = v
    geo_latest_vstring = 'v' + str(teeth_geo_latest_version).zfill(3)

    alembic_file_name = f"{shot}_{character}_{variant}_teeth_{geo_latest_vstring}.abc"
    teeth_geo_path = os.path.join(teeth_geo_dir, geo_latest_vstring, 'abc', alembic_file_name)

    # Make sure the gizmo is deselected
    node.setSelected(False)

    # Create the ReadGeo node and load the alembic file
    teeth_geo_node = import_alembic_animation_cache(filepath=teeth_geo_path, framerate=23.976)

    # Set the ReadGeo node position
    teeth_geo_node.knob('xpos').setValue(node.knob('xpos').value() - 100)
    teeth_geo_node.knob('ypos').setValue(node.knob('ypos').value() - 100)

    # Deselect teeth geo and reselect the gizmo
    teeth_geo_node.setSelected(False)
    node.setSelected(True)

    # connect teeth
    node.setInput(3, teeth_geo_node)
...
def main():
    node, neuralRender = gen_dataset(gizmo_path = '/Volumes/f1r2/pipeline/vfx/nuke/gizmos/Dataset_png_teeth_flw_v002.gizmo')
