...
class InternalRenderDataType():
    ...
    def prepare_auto_actions(self):
        [self.actions.insert(0, action) for action in
            GizmoActionLookup.compile_action_list("visibility", lambda : self.gizmo, [[node_name, "clearFlag"] for node_name in ["openFolder","filetype_flwls", "datatype_flwls", "filename_flwls","info_printout"]]) +\
    ...
    def resolve_afterRenderKnobContent(self):
        result = self.resolve_beforeRenderKnobContent() + "current_gizmo_pointer.knobs()['openFolder'].setEnabled(True);"
    ...
    def move_to_render_folder():
        ...
        for file in FlwlsVFXPipelinePanel.LogicEX.itera(FlwlsVFXPipelinePanel.InternalPubEX.get_latest_internal_render(source_folder)):
    ...
    def resolve_future_filepath(custom_name : str, ending : str) -> str:
        ...
        version_target = target / FlwlsVFXPipelinePanel.InternalPubEX.get_next_version(target)
...
def get_internal_render_data_type(name: str, gizmo : nuke.Node, overrides : Dict[Any,Any] = {}) -> InternalRenderDataType:
    lookup = {
        "SELECT" : InternalRenderDataType(
            ...
            actions = GizmoActionLookup.compile_action_list("visibility", lambda : gizmo, [
                    ["WriteGroupBegin", "setFlag"],["filetype_flwls" , "setFlag"],["filename_flwls", "setFlag"]]),),
        "PREPGEO" : InternalRenderDataType(
            ...
            actions = 
                ...
                GizmoActionLookup.compile_action_list("visibility", lambda : gizmo, [
                    ["WriteGroupBegin", "clearFlag"]] +\
