...
# where(filename) - looks in nuke.pluginPath() folders and returns strings with pathes contaning filename
...
def topInput(node,input,ch_class,knob,ch_frame):
    if node:
        ...
        if input_node:
            if ch_class in input_node.Class():
...
def topInputKnob(node,ch_class,knob,input=0):
    if node:
        input_node=node.input(input)
        if input_node:
            if ch_class in nodeClass(input_node):
                if input_node.knob(knob):
...
def topInputNode(node,ch_class,input=0):
    if node:
        input_node=node.input(input)
        if input_node:
            if nodeClass(input_node) in ch_class:
                return input_node
            else:
                if nodeClass(input_node)=='JoinViews':
#                    print nuke.views()
